...
class DSCWithBNAndRelu(Model):
    def __init__(self, indim, hidden, outdim, ksize, stride, padding, hasShortCut=False):
        super(DSCWithBNAndRelu, self).__init__()
        self.hasShortCut = hasShortCut
        self.conv1 = layers.Conv2D(hidden, (1, 1), use_bias=False)
        self.bn1 = layers.BatchNormalization()
        self.relu1 = layers.ReLU()
        self.dconv = layers.DepthwiseConv2D(ksize, strides=stride, padding=padding, use_bias=False)
        self.bn2 = layers.BatchNormalization()
        self.relu2 = layers.ReLU()
        self.conv2 = layers.Conv2D(outdim, (1, 1), use_bias=False)
...
class NIDS():
    def __init__(self,lossType):  # 不输入维度 默认是DATA_DIM
        ...
        self.features = Sequential([
            layers.Conv2D(32, kernel_size=(3, 3), strides=(2, 2), padding='same', input_shape=(8, 9, 1)),
            layers.BatchNormalization(),
            layers.ReLU(),
            DSCWithBNAndRelu(32, 32, 16, (3, 3), 1, 'same', hasShortCut=False),
            DSCWithBNAndRelu(16, 96, 24, (3, 3), 2, 'same', hasShortCut=False),
            DSCWithBNAndRelu(24, 144, 24, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(24, 144, 32, (3, 3), 2, 'same', hasShortCut=False),
            DSCWithBNAndRelu(32, 192, 32, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(32, 192, 32, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(32, 192, 64, (3, 3), 1, 'same', hasShortCut=False),
            DSCWithBNAndRelu(64, 384, 64, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(64, 384, 64, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(64, 384, 64, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(64, 384, 96, (3, 3), 2, 'same', hasShortCut=False),
            DSCWithBNAndRelu(96, 576, 96, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(96, 576, 96, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(96, 576, 160, (3, 3), 2, 'same', hasShortCut=False),
            DSCWithBNAndRelu(160, 960, 160, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(160, 960, 160, (3, 3), 1, 'same', hasShortCut=True),
            DSCWithBNAndRelu(160, 960, 320, (3, 3), 1, 'same', hasShortCut=False),
            layers.Conv2D(1280, kernel_size=(1, 1), use_bias=False),
            layers.BatchNormalization(),
            layers.ReLU(),
    ...
    def create_NIDS(self):
        scaled_input = tf.reshape(self.x_input, [-1, 8, 9, 1])
        self.y = self.features(scaled_input)
