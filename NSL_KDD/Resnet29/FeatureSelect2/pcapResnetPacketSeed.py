...
class ResNetModel2(Model):
    def __init__(self, dim, selected_features=[], seed=25):
        ...
        # Build the ResNet architecture - 使用更小的初始卷积层
        self.conv_layer = layers.Conv2D(32, kernel_size=(3, 3), strides=1, padding='same',
                                        kernel_initializer=tf.keras.initializers.glorot_uniform(seed=self.seed))
        self.stm = Sequential([
            self.conv_layer,
        ...
        # 使用更轻量级的架构减少内存使用
        layer_dims = [1, 1, 1, 1]  # 减少每层的块数
        self.layer1 = self.build_resblock(32, layer_dims[0])   # 减少滤波器数量
...
class Resnet2(): # 第二次训练
    ...
    def train_step(self, data, labels):
        try:
            # 将numpy数组转换为tensor，避免创建常量
            data_tensor = tf.convert_to_tensor(data, dtype=tf.float32)
            label_tensor = tf.convert_to_tensor(labels, dtype=tf.int32)
            
            with tf.GradientTape() as tape:
                predictions = self.model(data_tensor, training=True)
                loss = self.compute_loss(label_tensor, predictions)
    ...
    def train(self):
        ...
        for step in range(self.total_iterations_per_epoch):
            ...
            # 避免创建不必要的tensor常量，直接使用numpy数组
            loss = self.train_step(data, label)
            total_loss += loss.numpy()
            num_batches += 1
            
            # 每处理一定数量的批次后清理内存
            if step % 5 == 0:  # 更频繁的内存清理
                tf.keras.backend.clear_session()
                # 强制垃圾回收
                import gc
                gc.collect()
    ...
    def init_data(self):
        ...
        print(f"数据加载完成: 训练数据 {self.train_length} 条, 测试数据 {self.test_length} 条")
        print("开始数据标准化...")
