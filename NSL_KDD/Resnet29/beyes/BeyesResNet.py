# TensorFlow 2.9.0 compatible BeyesResNet implementation
import time
import numpy as np
import csv
import os
...
def objective(params):
    ...
    checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(model_dir, new_folder2, "model_best_epoch_{epoch:02d}.h5"),
        monitor='val_loss',
        save_best_only=True,
        mode='min'
    )
    ...
    for _ in range(TRAIN_EPOCH):
        ...
        if accuracy > best_accuracy:
            ...
            best_model_path = os.path.join(model_dir, new_folder2, f"model_best_epoch_{model2.epoch_count}.h5")
            model2.model.save(best_model_path)
    
    # Load best model
    if best_model_path and os.path.exists(best_model_path):
        model2.model = tf.keras.models.load_model(best_model_path)
