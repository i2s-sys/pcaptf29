...
class Resnet2(): # 第二次训练
    def __init__(self, dim, selected_features=[], seed=25):
        ...
        # Create the model
        self.model = ResNetModel(dim=len(selected_features), selected_features=selected_features, seed=self.seed)
    ...
    def predict_top_k(self, x_features, k=5):
        predictions = self.model(x_features, training=False)
        top_k_indices = [np.argsort(predict)[-k:][::-1] for predict in predictions.numpy()]
    ...
    def test(self):
        ...
        for step in range(num_batches):
            ...
            data_tensor = tf.constant(data, dtype=tf.float32)
            top_k_labels = self.predict_top_k(data_tensor, k=5)
