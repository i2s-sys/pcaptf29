# TensorFlow 2.9.0 compatible AutoEncoder + Random Forest for second phase training
import time
...
class AE2(Model):
    def __init__(self, dim, selected_features, seed):
        ...
        # Build AutoEncoder architecture with selected features
        self.build_encoder()
        self.build_decoder()
    ...
    def build_encoder(self):
        ...
        self.encoder = Sequential([
            ...
            layers.Dense(16, activation='relu', name='encoder_2',
                        kernel_initializer=tf.keras.initializers.GlorotUniform(seed=self.seed)),
            layers.Dropout(KEEP_PROB),
            layers.Dense(self.dim, activation='relu', name='encoder_3',
                        kernel_initializer=tf.keras.initializers.GlorotUniform(seed=self.seed))
        ])
    ...
    def build_decoder(self):
        ...
        self.decoder = Sequential([
            ...
            layers.Dense(32, activation='relu', name='decoder_2',
                        kernel_initializer=tf.keras.initializers.GlorotUniform(seed=self.seed)),
            layers.Dropout(KEEP_PROB),
            layers.Dense(self.dim, activation='sigmoid', name='decoder_3',
                        kernel_initializer=tf.keras.initializers.GlorotUniform(seed=self.seed))
        ])
    ...
    def init_data(self):
        """Initialize training and test data with selected features"""
        self.train_data = []
        self.test_data = []
        self.label_status = {}
