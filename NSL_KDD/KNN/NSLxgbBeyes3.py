...
def objective(params):
    n_features_to_select = 4  # 固定选择4个特征
    ...
    # 返回负的Macro-F1分数（因为Hyperopt默认是minimize）
    return {'loss': -macro_f1, 'status': STATUS_OK}

# 定义参数空间
param_space = {
    'n_estimators': hp.choice('n_estimators', [50, 100, 200]),  # XGBoost的树的数量
    'max_depth': hp.choice('max_depth', [3, 5, 7, 9, None])  # XGBoost的最大深度
}
