...
with model.sess as sess:
    ...
    print("top_k_factors",top_k_factors)
    print("bottom_k_factors",bottom_k_factors)
    ...
    # 绘制前K2个因子值随epoch变化的点图，用不同颜色的线连起来
    ...
    # 绘制后K2个因子值随epoch变化的点图，统一用棕色线
    plt.figure(figsize=(12, 6))
    for i in range(K2):
        plt.scatter(epochs, bottom_k_factors[:, i], label=f'Index {bottom_k_indices[i]}', color='brown')
    ...
    # 绘制前后K2个因子值随epoch变化的点图，前K个用不同颜色的线连起来，后K个统一用棕色线
    plt.figure(figsize=(12, 6))
    for i in range(K2):
        plt.scatter(epochs, top_k_factors[:, i], label=f'Index {top_k_indices[i]}')
        plt.plot(epochs, top_k_factors[:, i])
