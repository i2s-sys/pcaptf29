...
class Resnet():
    ...
    def create_ResNet(self):
        ...
        self.beta = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[1, DATA_DIM]))
    ...
    def build_loss(self):
        l1_regularizer = tf.keras.regularizers.l1(0.001)  # 比例因子
...
class Resnet2(): # 第二次训练
    ...
    def create_ResNet(self, NEW_DIM):
        # 缩放因子层
        # scaling_factor = tf.Variable(
        #     tf.ones(dtype=tf.float32, shape=[1, NEW_DIM]))
        # scaling_factor_extended = tf.tile(scaling_factor, [BATCH_SIZE, 1])
        # scaled_input = tf.multiply(self.x_input, scaling_factor_extended)
    ...
    def build_loss(self):
        ...
        # l1_regularizer = tf.contrib.layers.l1_regularizer(scale=0.01, scope=None)  # 比例因子
        # regularization_penalty = l1_regularizer(self.scaling_factor)
    ...
    def init_data(self): # 此处的init_data和第一个Resnet是不一样的，需要按照排序数组筛选
        self.train_data = []
        self.test_data = []  # init train and test data
        self.label_status = {}
        filename = 'train_data.csv'
