...
class Resnet():
    ...
    def train(self):
        ...
        if len(self.intersection_sets) > ESTHRESHOLD:  # 保持列表长度为ESTHRESHOLD
            self.intersection_sets.pop(0)
        if len(self.intersection_sets) == ESTHRESHOLD:  # 当列表中有ESTHRESHOLD个集合时，计算交集
            ...
            if len(intersection) == keyFeatureNums:
                print("Early stopping condition met.But not stop")
                self.earlyStop = True
