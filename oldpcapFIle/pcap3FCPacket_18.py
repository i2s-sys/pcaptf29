...
GAMMA = 1
Q = 5 # 连续5epoch下交集不再改变
THRESHOLD = 1  #每THRESHOLD次epoch就进行一次排序 看因子是否发生变化 如果连续2次不变化就早停
...
class DNN():
    ...
    def create_DNN(self):
        ...
        w1 = tf.Variable(tf.random.truncated_normal(dtype=tf.float32, shape=[DATA_DIM, NUM_HIDDEN1_NODES], stddev=0.1))
        b1 = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[NUM_HIDDEN1_NODES]))
        w2 = tf.Variable(
            tf.random.truncated_normal(dtype=tf.float32, shape=[NUM_HIDDEN1_NODES, NUM_HIDDEN2_NODES], stddev=0.1))
        b2 = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[NUM_HIDDEN2_NODES]))
        w3 = tf.Variable(
            tf.random.truncated_normal(dtype=tf.float32, shape=[NUM_HIDDEN2_NODES, NUM_HIDDEN3_NODES], stddev=0.1)) # NUM_HIDDEN3_NODES改成dim
        b3 = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[NUM_HIDDEN3_NODES]))
        w4 = tf.Variable(tf.random.truncated_normal(dtype=tf.float32, shape=[NUM_HIDDEN3_NODES, OUTPUT_DIM], stddev=0.1))
        ...
        h_layer = tf.nn.relu(tf.nn.dropout(tf.matmul(scaled_input, w1) + b1, KEEP_PROB))
    ...
    def build_loss(self):
        ...
        # grads_and_vars = self.optimizer.get_gradients(self.loss, [self.scaling_factor])
        # self.train_op = self.optimizer.apply_gradients(zip(grads_and_vars, [self.scaling_factor]),global_step=self.global_step)
    ...
    def train(self):
        ...
        if len(self.intersection_sets) > 5:  # 保持列表长度为5
            self.intersection_sets.pop(0)
        if len(self.intersection_sets) <= 5:  # 当列表中小于等于5个集合时，计算交集
...
class DNN2(): # 第二次训练
    ...
    def create_DNN(self, NEW_DIM):
        w1 = tf.Variable(tf.random.truncated_normal(dtype=tf.float32, shape=[NEW_DIM, NUM_HIDDEN1_NODES], stddev=0.1))
        b1 = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[NUM_HIDDEN1_NODES]))
        w2 = tf.Variable(
            tf.random.truncated_normal(dtype=tf.float32, shape=[NUM_HIDDEN1_NODES, NUM_HIDDEN2_NODES], stddev=0.1))
        b2 = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[NUM_HIDDEN2_NODES]))
        w3 = tf.Variable(
            tf.random.truncated_normal(dtype=tf.float32, shape=[NUM_HIDDEN2_NODES, NUM_HIDDEN3_NODES],
                                stddev=0.1))  # NUM_HIDDEN3_NODES改成dim
        b3 = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[NUM_HIDDEN3_NODES]))
        w4 = tf.Variable(tf.random.truncated_normal(dtype=tf.float32, shape=[NUM_HIDDEN3_NODES, OUTPUT_DIM], stddev=0.1))
        h_layer = tf.nn.relu(tf.nn.dropout(tf.matmul(self.x_input, w1) + b1, KEEP_PROB))
        h_layer2 = tf.nn.relu(tf.nn.dropout(tf.matmul(h_layer, w2) + b2, KEEP_PROB))
        h_layer3 = tf.nn.relu(tf.matmul(h_layer2, w3) + b3)
        self.y = tf.nn.softmax(tf.matmul(h_layer3, w4))
