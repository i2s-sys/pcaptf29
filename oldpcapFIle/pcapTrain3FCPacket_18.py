...
import numpy as np
import csv, os
from pcap3FCPacket_18 import DNN # 导入DNN类
from pcap3FCPacket_18 import DNN2 # 导入定义的DNN2类
...
# 训练模型
model.train_start = 0
for _ in range(100):  # 循环100次 # 调用train函数，并获取损失的变化
...
plt.ylabel('Loss')
if not os.path.exists('./topKAddNRecord/'):
    os.makedirs('./topKAddNRecord/')
plt.savefig('./topKAddNRecord/' + curr_time + '第一轮TrainPcapFLossHistory.png')
plt.clf()  # 清除当前图形 防止两张曲线绘制到一起
'''
model_dir = "./model"
...
with model.sess as sess:
    ...
    dnn2 = DNN2(dim=len(selected_features), selected_features=selected_features)
    print('start retraining...')

    start_time = time.time()
    for _ in range(100):  # 调用train函数，并获取损失的变化
        delta_loss, count = dnn2.train()  # 判断损失的变化是否小于阈值
        dnn2.epoch_count += 1
        if count >= 50:
            print("count >= 50: 连续5个epoch满足条件 变化不大 相差小于0.03")
            break
    end_time = time.time()  # 记录训练结束时间
    total_training_time = end_time - start_time  # 计算训练总时长
    print('start testing...')
