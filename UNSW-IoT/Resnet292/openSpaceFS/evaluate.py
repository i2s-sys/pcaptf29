...
2) 读取 CSV 数据（与 pcapResnetPacketSeed.py 的 init_data 相同格式：特征列 + 最后一列为整型标签）。
...
DEFAULT_DATA_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../test_data.csv"))
...
def main():
    ...
    parser.add_argument("--weights", type=str, default=None, help="模型权重路径（.weights.h5）。默认取 checkpoints 最新权重。")
    parser.add_argument("--output", type=str, default=DEFAULT_SAVE_DIR, help="结果保存目录。")
    parser.add_argument("--known_classes", type=int, default=20, help="封闭世界类别数（label < N 视为已知）。")
