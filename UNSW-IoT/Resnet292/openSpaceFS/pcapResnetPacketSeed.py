...
class Resnet2():
    ...
    def init_data(self):
        ...
        def process_row(row):
            data = [0 if char in ('None', '') else np.float32(char) for i, char in enumerate(row) if
        ...
        with open('../../OWtrain_data1.csv', mode="r", encoding="utf-8") as f:
        ...
        with open('../../OWtest_data1.csv', mode="r", encoding="utf-8") as f:
            csv_reader = csv.reader(f)
            self.test_data = [
                [0 if char in ('None', '') else np.float32(char) for i, char in enumerate(row) if
                 i in self.top_k_indices or i == len(row) - 1]
                for row in csv_reader
            ]
